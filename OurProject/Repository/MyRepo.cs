...
namespace OurProject.Repository
{
    public class MyRepo : ConnRepo
    {
        ...
        public Users DatafromUserTable(Usernameonly u1)
        {
            ...
            {
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "Select u.FName,u.MName,u.LName,u.Email,u.Username,u.Ward, e.PhoneNo,e.[Image] from Users u INNER JOIN ExtraUserData e ON u.Username=e.Username where u.Username = @name";
                using (SqlConnection conn1 = Connection())
                {
                    cmd.Connection = conn1;
                    cmd.CommandTimeout = 30;
                    ...
                    using (SqlDataReader sd = cmd.ExecuteReader())
                    {
                        if (sd.HasRows)
                        {
                            while (sd.Read())
                            {
                                ...
                                s2.Phone = Convert.ToString(sd["PhoneNo"]);
                                byte[] imageData = (byte[])sd["Image"];
                                using (MemoryStream ms = new MemoryStream(imageData, 0, imageData.Length))
                                {
                                    ms.Write(imageData, 0, imageData.Length);
                                    s2.Image = "data:image/png;base64," + Convert.ToBase64String(imageData);
                                }
                            }
                        }
                    }
